...
export default function MaintenancePage() {
  ...
  const [form, setForm] = useState({ vehicle_id:'', service_type:'', interval_miles:5000, interval_days:180, last_service_date:'', last_service_mileage:0, notes:'' })
  ...
  async function load() {
    const [m, v] = await Promise.all([fetch('/api/maintenance').then(r => r.json()), fetch('/api/fleet').then(r => r.json())])
  ...
  async function addCustom() {
    setSaving(true)
    await fetch('/api/maintenance', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({...form, status:'ok'}) })
            ...
            <div key={item.id} className="bg-white rounded-xl p-4 flex items-center gap-4"
              ...
              {isAlert && (
                <button onClick={() => markComplete(item)}
                  className="flex-shrink-0 flex items-center gap-1.5 text-xs px-3.5 py-2 rounded-lg font-medium transition-all hover:opacity-80"
